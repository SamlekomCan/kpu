...
$dataPoints2 = array();
$gubernurr = $this->db->get_where('calon', ['organisasi' => 'BEMF'])->result();
foreach ($gubernurr as $row) {
    array_push($dataPoints2, array("label" => ($row->ketua . ' - ' . $row->wakil), "y" => $row->hasil));
}

$dataPoints3 = array();
$himpunann = $this->db->get_where('calon', ['organisasi' => 'HM'])->result();

foreach ($himpunann as $row) {
